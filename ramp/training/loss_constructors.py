...
def get_class_tversky_fn(alpha, epsilon):
    ...
    def class_tversky(y_true, y_pred):
        ...
        return (true_pos + epsilon) / (
            true_pos + alpha * false_neg + (1 - alpha) * false_pos + epsilon
        )
...
def main():
    ...
    assert (
        np.sum(np.square(CT - truth)) < 10e-06
    ), f"returned unexpected value from class_tversky(): {CT}"
