...
# Patch for OHE : https://github.com/ciupava/ramp-code-fAIr/blob/7fee0ed584fcf7d93f558550ace299c21e9d60e2/ramp/data_mgmt/data_generator.py
#################################################################
...
def process_data(image, mask, img_shape, transforms):
    ...
    aug_img, aug_mask = tf.numpy_function(
        func=apply_aug_fn, inp=[image, mask], Tout=[tf.float32, tf.uint8]
    )
...
def training_batches_from_gtiff_dirs(
   ...
   """
    ...
    assert (
        n_images == n_masks
    ), f"Number of images: {n_images} | Number of masks: {n_masks}"
    ...
    tf_load_image_fn = lambda imgname: tf.py_function(
        func=tf_gdal_get_image_tensor, inp=[imgname], Tout=[tf.float32]
    )

    tf_load_mask_fn = lambda maskname: tf.py_function(
        func=tf_gdal_get_mask_tensor, inp=[maskname], Tout=[tf.uint8]
    )

    # resize the images and masks to the desired size.
    # num_classes = 4
    images = image_ds.map(tf_load_image_fn).map(
        lambda x: resize_it(x, [*input_image_size, 3], output_image_size, "bilinear")
    )
    masks = mask_ds.map(tf_load_mask_fn).map(
        lambda x: resize_it(x, [*input_image_size, 1], output_image_size, "nearest")
    )
    ...
    if transforms is not None:
        data_batches = (
            ...
            .map(
                partial(
                    process_data, img_shape=output_image_size, transforms=transforms
                ),
                num_parallel_calls=tf.data.AUTOTUNE,
            )
            .map(
                partial(reset_shapes, image_shape=output_image_size),
                num_parallel_calls=tf.data.AUTOTUNE,
            )
            # END augmentation
            .batch(batch_size)
...
def test_batches_from_gtiff_dirs(
    image_file_dir, mask_file_dir, batch_size, input_image_size, output_image_size
):
    """
    ...
    data_batches = zip_pairs.batch(batch_size)
...
def test_batches_from_gtiff_dirs_with_names(
    ...
    # the gdal_get_image_tensor and gdal_get_mask_tensor functions.
    tf_load_image_fn = lambda imgname: tf.py_function(
        func=tf_gdal_get_image_tensor_with_name,
        inp=[imgname],
        Tout=[tf.float32, tf.string],
    )

    tf_load_mask_fn = lambda maskname: tf.py_function(
        func=tf_gdal_get_mask_tensor_with_name,
        inp=[maskname],
    ...
    # resize the images and masks to the desired size.
    images = image_ds.map(tf_load_image_fn).map(
        lambda x, pathname: resize_it_with_name(
            x, [*input_image_size, 3], output_image_size, "bilinear", pathname
        )
    )
    masks = mask_ds.map(tf_load_mask_fn).map(
        lambda x, pathname: resize_it_with_name(
            x, (*input_image_size, 1), output_image_size, "nearest", pathname
        )
    )
